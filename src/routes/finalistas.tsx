...
export const Route = createFileRoute("/finalistas")({
  head: () => ({ meta: [{ title: "Finalistas — Bolão Copa 2026" }, { name: "description", content: "Aposte nos dois finalistas da Copa 2026." }] }),
  component: Page,
});
...
function Page() {
  ...
  const enviar = useMutation({
    mutationFn: () => callFn("aposta-finalistas", { nome: identidade?.nome, pin: identidade?.pin, time1: t1, time2: t2 }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["apostas-fin"] }); toast.success("Aposta registrada!"); },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {!aberta && cfg?.status === "fechada" && (
        <Card><CardContent className="py-6 text-center text-muted-foreground">As apostas abrem automaticamente quando o primeiro jogo das oitavas for agendado.</CardContent></Card>
      )}

      {aberta && (
        <Card>
          <CardHeader><CardTitle>Seu palpite</CardTitle></CardHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={t2} onValueChange={setT2}>
                  <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                  <SelectContent>
                    {times?.filter(t => t !== t1).map(t => <SelectItem key={t} value={t}>{flag(t)} {t}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <Button onClick={() => enviar.mutate()} disabled={!identidade?.nome || !t1 || !t2 || enviar.isPending} className="w-full">
              Registrar aposta
            </Button>
          </CardContent>
        </Card>
