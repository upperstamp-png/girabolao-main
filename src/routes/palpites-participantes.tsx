...
function Page() {
    ...
    (usuarios ?? []).forEach(u => {
      ...
      for (const p of userPalpites) {
        ...
        if (g && g.placar_casa != null && g.placar_fora != null) {
          pontos += calcularPontosPalpite(p.gols_casa, p.gols_fora, g.placar_casa, g.placar_fora).pontos;
        }
      }
      if (!!(apostasArt ?? []).find(a => a.usuario_id === u.id)?.acertou) pontos += 10;
      const apostaFin = (apostasFin ?? []).find(a => a.usuario_id === u.id);
      if (apostaFin) { if (apostaFin.acertou_os_dois) pontos += 10; else if (apostaFin.acertou_um) pontos += 5; }
  ...
  const bolaoFechado = useMemo(() => config?.status === "FECHADO" || config?.status === "FINALIZADO", [config]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setTimeRemaining("Encerrado"); setIsUrgent(false); return; }
  ...
  const jogoEscolhidoObj = useMemo(() => jogos?.find(j => j.id === selectedJogo), [selectedJogo, jogos]);
    ...
    <div className="max-w-4xl mx-auto pb-20 animate-in space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-xs mt-0.5">Compare as previsões de todos os participantes</p>
        </div>
      </div>

      {/* Status banner */}
      <div className={`flex items-center justify-between p-3 rounded-xl border text-xs gap-3 ${
        bolaoFechado
        ...
        <div className="flex items-center gap-2 min-w-0">
          {bolaoFechado ? <Lock className="h-3.5 w-3.5 shrink-0" /> : <Unlock className="h-3.5 w-3.5 shrink-0" />}
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { emoji: "🎯", label: "Placar Exato", pts: "+3 pts", color: "border-yellow-500/30 bg-yellow-500/5 text-yellow-400" },
          { emoji: "⚽", label: "Vencedor/Empate", pts: "+1 pt", color: "border-primary/30 bg-primary/5 text-primary" },
          { emoji: "❌", label: "Errou", pts: "0 pts", color: "border-border bg-secondary/10 text-muted-foreground" },
        ].map(item => (
          <div key={item.label} className={`flex flex-col items-center gap-1 p-2.5 rounded-xl border text-center ${item.color}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-center">
                      ...
                      <div className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mt-0.5">pontos totais</div>
                      <div className="text-sm font-bold text-foreground mt-2">{nomeMap.get(selectedUser) ?? "—"}</div>
                    </div>
                    ...
                    <Card className="border-border">
                      ...
                      <CardContent className="p-3 grid grid-cols-2 gap-2 text-xs">
                        {[
                          ...
                          { label: "🥇 Campeão", value: userEspeciais?.cam?.time_campeao ? `${flag(userEspeciais.cam.time_campeao)} ${userEspeciais.cam.time_campeao}` : null },
                          {
                            label: "🏆 Finalistas", span: true,
                            value: userEspeciais?.fin?.time1 && userEspeciais?.fin?.time2
                              ? `${flag(userEspeciais.fin.time1)} ${userEspeciais.fin.time1} × ${flag(userEspeciais.fin.time2)} ${userEspeciais.fin.time2}`
                              : null
                          },
                          { label: "🦓 Zebra", value: userEspeciais?.zeb?.zebra_apostada ? `${flag(userEspeciais.zeb.zebra_apostada)} ${userEspeciais.zeb.zebra_apostada}` : null },
                          {
                            label: "🔥 Goleada",
                            value: userEspeciais?.gol?.time_casa && userEspeciais?.gol?.time_fora
                          ...
                          <div key={item.label} className={`p-2 rounded-lg border border-border/30 bg-secondary/10 flex flex-col gap-0.5 ${item.span ? "col-span-2" : ""}`}>
                            <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-bold">{item.label}</span>
                            <span className="font-semibold text-[11px] truncate">{item.value || "—"}</span>
                          </div>
                        ))}
                      </CardContent>
                    </Card>
                  ...
                  <div className="md:col-span-2">
                    <Card className="border-border h-full">
                      <CardHeader className="py-2.5 px-3 border-b border-border/30">
                        <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Palpites nos Jogos</CardTitle>
                      </CardHeader>
                      <CardContent className="p-2">
                        <div className="space-y-1.5 max-h-[480px] overflow-y-auto pr-1">
                          {(jogos ?? []).map(j => {
                            ...
                            const oficialStr = j.placar_casa != null && j.placar_fora != null
                            ...
                            return (
                              <div key={j.id} className="flex items-center gap-2 p-2.5 rounded-xl border border-border/30 hover:bg-secondary/15 transition-all text-xs">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <div className="text-right">
                                    <div className="text-[9px] text-muted-foreground uppercase font-bold">Palpite</div>
                                    <div className="font-mono font-bold">{palpStr}</div>
                                  </div>
                                  <div className="text-center">
                                    <div className="text-[9px] text-muted-foreground uppercase font-bold">Placar</div>
                                    <div className="font-mono text-[11px] text-muted-foreground bg-secondary/30 px-1.5 py-0.5 rounded">{oficialStr}</div>
                                  </div>
                                  <div className="w-12 text-right">
                                    {badge ? (
                                      <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold border ${badge.cls}`}>
                                        {badge.text}
                                      </span>
                                    ) : p ? (
                                      <span className="text-[9px] text-muted-foreground italic">Aguard.</span>
                                    ) : null}
                                  </div>
                                </div>
                              </div>
                          ...
                          {(jogos ?? []).length === 0 && (
                            <p className="text-xs text-muted-foreground text-center py-8">Nenhum jogo cadastrado.</p>
                          )}
                        </div>
                      </CardContent>
                    </Card>
              ...
              {selectedJogo && jogoEscolhidoObj && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <Card className="border-border h-full">
                      <CardHeader className="py-2.5 px-3 border-b border-border/30">
                        <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Palpites da Galera</CardTitle>
                      </CardHeader>
                      <CardContent className="p-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-[480px] overflow-y-auto pr-1">
                          {(usuarios ?? []).map(u => {
                            ...
                            return (
                              <div key={u.id} className={`flex items-center justify-between p-2.5 rounded-xl border transition-all text-xs ${
                                isMe ? "border-primary/40 bg-primary/5" : "border-border/30 hover:bg-secondary/15"
                              }`}>
                                <span className={`font-semibold truncate mr-2 ${isMe ? "text-primary" : "text-foreground"}`}>
                                  {u.nome}{isMe ? " • Você" : ""}
                                </span>
                                <div className="flex items-center gap-2 shrink-0">
