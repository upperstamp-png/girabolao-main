...
export const Route = createFileRoute("/artilheiro")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Aposte no artilheiro da Copa 2026." },
    ],
  }),
  component: Page,
});
...
function Page() {
  ...
  const enviar = useMutation({
    mutationFn: () =>
      callFn("aposta-artilheiro", { nome: identidade?.nome, pin: identidade?.pin, jogador }),
    onSuccess: () => {
      setJogador("");
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          R$10 por participante. Aposte antes do primeiro jogo da Copa.
        </p>
      </div>
      ...
      <Card className="bg-pitch border-primary/20">
        <CardContent className="py-6 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1 mb-2">
            Para quem acertar o artilheiro oficial da Copa
          </p>
          <div className="flex justify-center gap-2 flex-wrap">
            ...
            {cfg?.artilheiro_real && (
              <Badge className="bg-gold-gradient text-black">🏆 {cfg.artilheiro_real}</Badge>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      {aberta && (
        <Card>
          <CardHeader>
            <CardTitle>Seu palpite de artilheiro</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <Button
              ...
              disabled={!identidade?.nome || !jogador.trim() || enviar.isPending}
              className="w-full"
            >
              Registrar aposta
            </Button>
          ...
          {(apostas?.length ?? 0) === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {apostas!.map((a: any) => (
                <li
                  key={a.id}
                  className={`flex justify-between py-2 ${a.acertou ? "text-success font-bold" : ""}`}
                >
                  <span>{nomeMap.get(a.usuario_id) ?? "—"}</span>
                  <span>
                    {a.jogador_apostado || "Sem aposta"}
                    {a.acertou && " ✓"}
                  </span>
                </li>
              ))}
