...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: apostas } = await supabase
      ...
      .select("id, usuario_id, jogador_apostado");
    ...
    const acertadores = (apostas ?? []).filter(
      (a) => a.jogador_apostado.toLowerCase() === artilheiro.toLowerCase(),
    );
